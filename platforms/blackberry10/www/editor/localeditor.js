...
var webodfEditor = (function () {
    ...
    function loadFromBuffer(fileName,filebytes)
    {
    //    editorInstance.endEditing();
       // editorInstance.closeSession();
        //editorInstance.closeDocument();
         if(editorInstance != null)
         {
           ...
           editorInstance.closeDocument(function() {
               runtime.registerFile(fileName, filebytes);
    ...
    function fileSelectHandler(evt) {
        ...
        function onLoadEnd() {
            if (reader.readyState === 2) {
                alert('fileSelectHandler' + reader.result);
    ...
    function createFileLoadForm() {
        ...
        function internalHandler(evt) {
            if (input.value !== "") {
                alert('internal file handler' + evt.toString());
    ...
    function save() {
        editorInstance.saveDocument(FILE_PATH);
    }
...
//             };
        ...
        //editorInstance = new Editor11(editorOptions);
        //dojo.require('editor.Editor');
        require({ }, ["editor/Editor"],
                      function(Editor) {
                          editorInstance = new Editor(editorOptions);
                          if(isLoaded == false)
                          {
                              loadFromBuffer(tempFileName,tempFileBytes);
                          }
            });

        //editorInstance = new Editor(editorOptions);
///*       JavaScript1.load("/editor/Editor.js", function() {
//           editorInstance = new Editor(editorOptions);
//       });*/
...
*/
    ...
    // exposed API
    return { boot: boot, loadFromBuffer:loadFromBuffer,save:save};
}());
